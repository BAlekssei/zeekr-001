...
class ZeekrSideslip @JvmOverloads constructor(
  context: Context,
    ...
    try {
      ...
      for (i in 0 until indexCount) {
        when (typedArray.getIndex(i)) {
          R.styleable.ZeekrSlider_leftMenuView -> {
            mLeftViewResID =
              typedArray.getResourceId(R.styleable.ZeekrSlider_leftMenuView, -1)
          }
          R.styleable.ZeekrSlider_rightMenuView -> {
            mRightViewResID =
              typedArray.getResourceId(R.styleable.ZeekrSlider_rightMenuView, -1)
          }
          R.styleable.ZeekrSlider_contentView -> {
            mContentViewResID =
              typedArray.getResourceId(R.styleable.ZeekrSlider_contentView, -1)
          }
          R.styleable.ZeekrSlider_canLeftSwipe -> {
            isCanLeftSwipe =
              typedArray.getBoolean(R.styleable.ZeekrSlider_canLeftSwipe, true)
          }
          R.styleable.ZeekrSlider_canRightSwipe -> {
            isCanRightSwipe =
              typedArray.getBoolean(R.styleable.ZeekrSlider_canRightSwipe, true)
          }
          R.styleable.ZeekrSlider_fraction -> {
            fraction = typedArray.getFloat(R.styleable.ZeekrSlider_fraction, 0.5f)
          }
        }
      }
    } catch (e: Exception) {
  ...
  override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
    when (ev.action) {
      ...
      MotionEvent.ACTION_MOVE -> {
        ...
        if (abs(distanceX) <= mScaledTouchSlop) { return false }
      ...
      MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
        ...
        if (abs(finalyDistanceX) > mScaledTouchSlop) {
          isSwipeing = true
        }
  ...
  private fun isShouldOpen(scrollX: Int): State? {
    if (mScaledTouchSlop >= abs(finalyDistanceX)) {
      return mStateCache
    }
    ...
    if (finalyDistanceX < 0) {
      ...
      // 3、开启右边菜单按钮
      if (getScrollX() > 0 && mRightView != null) {
        if (abs(mRightView!!.width * fraction) < abs(getScrollX())) {
  ...
  private val isLeftToRight: Boolean
    private get() = distanceX < 0

  companion object {
    private const val TAG = "ZeekrSideslip"
    var viewCache: ZeekrSideslip? = null
      private set
