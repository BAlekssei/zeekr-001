...
import static com.zeekr.component.refresh.util.ZeekrSmartUtil.dp2px;
import static com.zeekr.component.refresh.util.ZeekrSmartUtil.fling;
...
import com.zeekr.component.refresh.listener.ZeekrOnLoadMoreListener;
import com.zeekr.component.refresh.listener.ZeekrOnMultiListener;
import com.zeekr.component.refresh.listener.ZeekrOnRefreshListener;
import com.zeekr.component.refresh.listener.ZeekrOnRefreshLoadMoreListener;
...
import com.zeekr.component.refresh.wrapper.ZeekrRefreshFooterWrapperSimpleComponent;
import com.zeekr.component.refresh.wrapper.ZeekrRefreshHeaderWrapperSimpleComponent;
...
public class ZeekrRefreshLayout extends ViewGroup implements com.zeekr.component.refresh.api.ZeekrRefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
    ...
    protected float mDragRate = .5f; // 越小阻尼越大
    ...
    // 是否开启下拉回弹提示
    protected boolean mEnableOverScrollDownTips = false;

    // 是否开启上拉回弹提示
    protected boolean mEnableOverScrollUpTips = false;
    ...
    protected ZeekrOnRefreshListener mRefreshListener;
    protected ZeekrOnLoadMoreListener mLoadMoreListener;
    protected ZeekrOnMultiListener mOnMultiListener;
    protected ZeekrScrollBoundaryDecider mScrollBoundaryDecider;
    //</editor-fold>
    ...
    protected ZeekrDimensionStatus mHeaderHeightStatus = ZeekrDimensionStatus.DefaultUnNotify;
    protected int mFooterHeight;        //底部高度 和 底部高度状态
    protected ZeekrDimensionStatus mFooterHeightStatus = ZeekrDimensionStatus.DefaultUnNotify;

    protected int mHeaderInsetStart;    // Header 起始位置偏移
    protected int mFooterInsetStart;    // Footer 起始位置偏移

    protected float mHeaderMaxDragRate = 2.5f;  //最大拖动比率(最大高度/Header高度) 要求大于1，越小阻尼越大
    ...
    protected ZeekrRefreshComponent mRefreshHeader;     //下拉头部视图
    protected ZeekrRefreshComponent mRefreshFooter;     //上拉底部视图
    protected ZeekrRefreshContent mZeekrRefreshContent;   //显示内容视图
    //</editor-fold>

    protected Paint mPaint;
    protected Handler mHandler;
    protected ZeekrRefreshKernel mKernel = new ZeekrRefreshKernelImpl();

    /**
     * 【主要状态】
     * 面对 SmartRefresh 外部的滚动状态
     */
    protected ZeekrRefreshState mState = ZeekrRefreshState.None;          //主状态
    /**
     * 【附加状态】
     ...
     */
    protected ZeekrRefreshState mViceState = ZeekrRefreshState.None;      //副状态（主状态刷新时候的滚动状态）
    ...
    protected static ZeekrDefaultRefreshFooterCreator sFooterCreator = null;
    protected static ZeekrDefaultRefreshHeaderCreator sHeaderCreator = null;
    protected static ZeekrDefaultRefreshInitializer sRefreshInitializer = null;
    ...
    public ZeekrRefreshLayout(Context context) {
        this(context, null);
    }

    public ZeekrRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        mReboundInterpolator = new ZeekrSmartUtil(ZeekrSmartUtil.INTERPOLATOR_VISCOUS_FLUID);
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ZeekrRefreshLayout);

        if (!ta.hasValue(R.styleable.ZeekrRefreshLayout_android_clipToPadding)) {
            super.setClipToPadding(false);
        }
        if (!ta.hasValue(R.styleable.ZeekrRefreshLayout_android_clipChildren)) {
        ...
        // 获取设置的惯性最小的阈值速度
        mMinimumVelocity = ta.getInt(R.styleable.ZeekrRefreshLayout_zeekrSpringMinimumVelocity, mMinimumVelocity);
        mDragRate = ta.getFloat(R.styleable.ZeekrRefreshLayout_zeekrRLDragRate, mDragRate);
        mHeaderMaxDragRate = ta.getFloat(R.styleable.ZeekrRefreshLayout_zeekrRLHeaderMaxDragRate, mHeaderMaxDragRate);
        mFooterMaxDragRate = ta.getFloat(R.styleable.ZeekrRefreshLayout_zeekrRLFooterMaxDragRate, mFooterMaxDragRate);
        mHeaderTriggerRate = ta.getFloat(R.styleable.ZeekrRefreshLayout_zeekrRLHeaderTriggerRate, mHeaderTriggerRate);
        mFooterTriggerRate = ta.getFloat(R.styleable.ZeekrRefreshLayout_zeekrRLFooterTriggerRate, mFooterTriggerRate);
        mEnableRefresh = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableRefresh, mEnableRefresh);
        mReboundDuration = ta.getInt(R.styleable.ZeekrRefreshLayout_zeekrRLReboundDuration, mReboundDuration);
        mEnableLoadMore = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableLoadMore, mEnableLoadMore);
        mHeaderHeight = ta.getDimensionPixelOffset(R.styleable.ZeekrRefreshLayout_zeekrRLHeaderHeight, mHeaderHeight);
        mFooterHeight = ta.getDimensionPixelOffset(R.styleable.ZeekrRefreshLayout_zeekrRLFooterHeight, mFooterHeight);
        mHeaderInsetStart = ta.getDimensionPixelOffset(R.styleable.ZeekrRefreshLayout_zeekrRLHeaderInsetStart, mHeaderInsetStart);
        mFooterInsetStart = ta.getDimensionPixelOffset(R.styleable.ZeekrRefreshLayout_zeekrRLFooterInsetStart, mFooterInsetStart);
        mDisableContentWhenRefresh = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLDisableContentWhenRefresh, mDisableContentWhenRefresh);
        mDisableContentWhenLoading = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLDisableContentWhenLoading, mDisableContentWhenLoading);
        mEnableHeaderTranslationContent = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableHeaderTranslationContent, mEnableHeaderTranslationContent);
        mEnableFooterTranslationContent = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableFooterTranslationContent, mEnableFooterTranslationContent);
        mEnablePreviewInEditMode = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnablePreviewInEditMode, mEnablePreviewInEditMode);
        mEnableAutoLoadMore = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableAutoLoadMore, mEnableAutoLoadMore);
        mEnableOverScrollBounce = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableOverScrollBounce, mEnableOverScrollBounce);
        mEnablePureScrollMode = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnablePureScrollMode, mEnablePureScrollMode);
        mEnableScrollContentWhenLoaded = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableScrollContentWhenLoaded, mEnableScrollContentWhenLoaded);
        mEnableScrollContentWhenRefreshed = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableScrollContentWhenRefreshed, mEnableScrollContentWhenRefreshed);
        mEnableLoadMoreWhenContentNotFull = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableLoadMoreWhenContentNotFull, mEnableLoadMoreWhenContentNotFull);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableFooterFollowWhenLoadFinished, mEnableFooterFollowWhenNoMoreData);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableFooterFollowWhenNoMoreData, mEnableFooterFollowWhenNoMoreData);
        mEnableClipHeaderWhenFixedBehind = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableClipHeaderWhenFixedBehind, mEnableClipHeaderWhenFixedBehind);
        mEnableClipFooterWhenFixedBehind = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableClipFooterWhenFixedBehind, mEnableClipFooterWhenFixedBehind);
        mEnableOverScrollDrag = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableOverScrollDrag, mEnableOverScrollDrag);
        mFixedHeaderViewId = ta.getResourceId(R.styleable.ZeekrRefreshLayout_zeekrRLFixedHeaderViewId, mFixedHeaderViewId);
        mFixedFooterViewId = ta.getResourceId(R.styleable.ZeekrRefreshLayout_zeekrRLFixedFooterViewId, mFixedFooterViewId);
        mHeaderTranslationViewId = ta.getResourceId(R.styleable.ZeekrRefreshLayout_zeekrRLHeaderTranslationViewId, mHeaderTranslationViewId);
        mFooterTranslationViewId = ta.getResourceId(R.styleable.ZeekrRefreshLayout_zeekrRLFooterTranslationViewId, mFooterTranslationViewId);
        mEnableNestedScrolling = ta.getBoolean(R.styleable.ZeekrRefreshLayout_zeekrRLEnableNestedScrolling, mEnableNestedScrolling);
        mNestedChild.setNestedScrollingEnabled(mEnableNestedScrolling);

        mManualLoadMore = mManualLoadMore || ta.hasValue(R.styleable.ZeekrRefreshLayout_zeekrRLEnableLoadMore);
        mManualHeaderTranslationContent = mManualHeaderTranslationContent || ta.hasValue(R.styleable.ZeekrRefreshLayout_zeekrRLEnableHeaderTranslationContent);
        mManualFooterTranslationContent = mManualFooterTranslationContent || ta.hasValue(R.styleable.ZeekrRefreshLayout_zeekrRLEnableFooterTranslationContent);
        mHeaderHeightStatus = ta.hasValue(R.styleable.ZeekrRefreshLayout_zeekrRLHeaderHeight) ? ZeekrDimensionStatus.XmlLayoutUnNotify : mHeaderHeightStatus;
        mFooterHeightStatus = ta.hasValue(R.styleable.ZeekrRefreshLayout_zeekrRLFooterHeight) ? ZeekrDimensionStatus.XmlLayoutUnNotify : mFooterHeightStatus;

        int accentColor = ta.getColor(R.styleable.ZeekrRefreshLayout_zeekrRLAccentColor, 0);
        int primaryColor = ta.getColor(R.styleable.ZeekrRefreshLayout_zeekrRLPrimaryColor, 0);
        if (primaryColor != 0) {
            if (accentColor != 0) {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (isContentView(view) && (contentLevel < 2 || i == 1)) {
                indexContent = i;
                contentLevel = 2;
            } else if (!(view instanceof ZeekrRefreshComponent) && contentLevel < 1) {
                indexContent = i;
        ...
        int indexFooter = -1;
        if (indexContent >= 0) {
            mZeekrRefreshContent = new ZeekrRefreshContentWrapper(super.getChildAt(indexContent));
            if (indexContent == 1) {
                indexHeader = 0;
                if (count == 3) {
                    indexFooter = 2;
        ...
        for (int i = 0; i < count; i++) {
            View view = super.getChildAt(i);
            if (i == indexHeader || (i != indexFooter && indexHeader == -1 && mRefreshHeader == null && view instanceof ZeekrRefreshHeader)) {
                mRefreshHeader = (view instanceof ZeekrRefreshHeader) ? (ZeekrRefreshHeader) view : new ZeekrRefreshHeaderWrapperSimpleComponent(view);
            } else if (i == indexFooter || (indexFooter == -1 && view instanceof ZeekrRefreshFooter)) {
                mEnableLoadMore = (mEnableLoadMore || !mManualLoadMore);
                mRefreshFooter = (view instanceof ZeekrRefreshFooter) ? (ZeekrRefreshFooter) view : new ZeekrRefreshFooterWrapperSimpleComponent(view);
            }
        }
    ...
    protected void onAttachedToWindow() {
        ...
        if (!thisView.isInEditMode()) {
            ...
            if (mRefreshHeader == null) {
                if (sHeaderCreator != null) {
                    ZeekrRefreshHeader header = sHeaderCreator.createRefreshHeader(thisView.getContext(), this);
            ...
            if (mRefreshFooter == null) {
                if (sFooterCreator != null) {
                    ZeekrRefreshFooter footer = sFooterCreator.createRefreshFooter(thisView.getContext(), this);
                    //noinspection ConstantConditions
                    if (footer == null) {
            ...
            if (mZeekrRefreshContent == null) {
                ...
                mZeekrRefreshContent = new ZeekrRefreshContentWrapper(errorView);
                mZeekrRefreshContent.getView().setPadding(padding, padding, padding, padding);
            }
            ...
            if (mSpinner != 0) {
                notifyStateChanged(ZeekrRefreshState.None);
                mZeekrRefreshContent.moveSpinner(mSpinner = 0, mHeaderTranslationViewId, mFooterTranslationViewId);
            }
        ...
        //重新排序
        if (mZeekrRefreshContent != null) {
            super.bringChildToFront(mZeekrRefreshContent.getView());
        }
     ...
     * 1.测量代码看起来很复杂，时因为 Header Footer 有四种拉伸变换样式 {@link ZeekrSpinnerStyle}，每一种样式有自己的测量方法
    ...
    protected void onMeasure(final int widthMeasureSpec,final int heightMeasureSpec) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (child.getVisibility() == GONE || "GONE".equals(child.getTag(R.id.zrl_tag))) {
                continue;
            }
            ...
            if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
                ...
                if (mHeaderHeightStatus.ordinal < ZeekrDimensionStatus.XmlLayoutUnNotify.ordinal) {
                    if (lp.height > 0) {
                        height =  lp.height + mlp.bottomMargin + mlp.topMargin;
                        if (mHeaderHeightStatus.canReplaceWith(ZeekrDimensionStatus.XmlExactUnNotify)) {
                            mHeaderHeight = lp.height + mlp.bottomMargin + mlp.topMargin;
                            mHeaderHeightStatus = ZeekrDimensionStatus.XmlExactUnNotify;
                        }
                    } else if (lp.height == WRAP_CONTENT && (mRefreshHeader.getSpinnerStyle() != ZeekrSpinnerStyle.MATCH_LAYOUT || !mHeaderHeightStatus.notified)) {
                        ...
                        if (measuredHeight > 0) {
                            height = -1;
                            if (measuredHeight != (maxHeight) && mHeaderHeightStatus.canReplaceWith(ZeekrDimensionStatus.XmlWrapUnNotify)) {
                                mHeaderHeight = measuredHeight + mlp.bottomMargin + mlp.topMargin;
                                mHeaderHeightStatus = ZeekrDimensionStatus.XmlWrapUnNotify;
                            }
                        }
                    }
                }

                if (mRefreshHeader.getSpinnerStyle() == ZeekrSpinnerStyle.MATCH_LAYOUT) {
            ...
            if (mRefreshFooter != null && mRefreshFooter.getView() == child) {
                ...
                if (mFooterHeightStatus.ordinal < ZeekrDimensionStatus.XmlLayoutUnNotify.ordinal) {
                    if (lp.height > 0) {
                        height = lp.height + mlp.topMargin + mlp.bottomMargin;
                        if (mFooterHeightStatus.canReplaceWith(ZeekrDimensionStatus.XmlExactUnNotify)) {
                            mFooterHeight = lp.height + mlp.topMargin + mlp.bottomMargin;
                            mFooterHeightStatus = ZeekrDimensionStatus.XmlExactUnNotify;
                        }
                    } else if (lp.height == WRAP_CONTENT && (mRefreshFooter.getSpinnerStyle() != ZeekrSpinnerStyle.MATCH_LAYOUT || !mFooterHeightStatus.notified)) {
                        ...
                        if (measuredHeight > 0) {
                            height = -1;
                            if (measuredHeight != (maxHeight) && mFooterHeightStatus.canReplaceWith(ZeekrDimensionStatus.XmlWrapUnNotify)) {
                                mFooterHeight = measuredHeight + mlp.topMargin + mlp.bottomMargin;
                                mFooterHeightStatus = ZeekrDimensionStatus.XmlWrapUnNotify;
                            }
                        }
                    }
                }

                if (mRefreshFooter.getSpinnerStyle() == ZeekrSpinnerStyle.MATCH_LAYOUT) {
                    height = getSize(heightMeasureSpec);
            ...
            if (mZeekrRefreshContent != null && mZeekrRefreshContent.getView() == child) {
                final View contentView = mZeekrRefreshContent.getView();
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
                ...
                if (!isPreviewMode) {
                    if (mRefreshHeader.getSpinnerStyle() == ZeekrSpinnerStyle.TRANSLATE) {
                        top = top - mHeaderHeight;
                        bottom = bottom - mHeaderHeight;
                        /*
...
//                        bottom = top + Math.max(Math.max(0, isEnableRefreshOrLoadMore(mEnableRefresh) ? mSpinner : 0) - lp.bottomMargin - lp.topMargin, 0);
            ...
            if (mRefreshFooter != null && mRefreshFooter.getView() == child) {
                ...
                final ViewGroup.LayoutParams lp = footerView.getLayoutParams();
                final MarginLayoutParams mlp = lp instanceof MarginLayoutParams ? (MarginLayoutParams)lp : sDefaultMarginLP;
                final ZeekrSpinnerStyle style = mRefreshFooter.getSpinnerStyle();
                int left = mlp.leftMargin;
                int top = mlp.topMargin + thisView.getMeasuredHeight() - mFooterInsetStart;
                if (mFooterNoMoreData && mFooterNoMoreDataEffective && mEnableFooterFollowWhenNoMoreData && mZeekrRefreshContent != null
                        && mRefreshFooter.getSpinnerStyle() == ZeekrSpinnerStyle.TRANSLATE
                ...
                if (style == ZeekrSpinnerStyle.MATCH_LAYOUT) {
                    top = mlp.topMargin - mFooterInsetStart;
                } else if (isPreviewMode
                        || style == ZeekrSpinnerStyle.FIXED_FRONT
                        || style == ZeekrSpinnerStyle.FIXED_BEHIND) {
    ...
    protected void onDetachedFromWindow() {
         ...
         */
        if (mRefreshHeader != null && mState == ZeekrRefreshState.Refreshing) {
            mRefreshHeader.onFinish(this, false);
        }
        if (mRefreshFooter != null && mState == ZeekrRefreshState.Loading) {
            mRefreshFooter.onFinish(this, false);
        }
        if (mSpinner != 0) {
            mKernel.moveSpinner(0, true);
        }
        if (mState != ZeekrRefreshState.None) {
            notifyStateChanged(ZeekrRefreshState.None);
        }
        if (mHandler != null) {
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        final View thisView = this;
        final View contentView = mZeekrRefreshContent != null ? mZeekrRefreshContent.getView() : null;
        if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
            ...
            if (contentView != null) {
                ...
                if (mHeaderBackgroundColor != 0 && mPaint != null) {
                    ...
                    } else if (mRefreshHeader.getSpinnerStyle() == ZeekrSpinnerStyle.TRANSLATE) {
                 ...
                 */
                if ((mEnableClipHeaderWhenFixedBehind && mRefreshHeader.getSpinnerStyle() == ZeekrSpinnerStyle.FIXED_BEHIND) || mRefreshHeader.getSpinnerStyle().scale) {
                    canvas.save();
            ...
            if (contentView != null) {
                ...
                if (mFooterBackgroundColor != 0 && mPaint != null) {
                    ...
                    if (mRefreshFooter.getSpinnerStyle().scale) {
                        top = child.getTop();
                    } else if (mRefreshFooter.getSpinnerStyle() == ZeekrSpinnerStyle.TRANSLATE) {
                 ...
                 * 并且取消了对 child 的绘制裁剪，所以经典组件需要重写 dispatchDraw 自行裁剪
                 */
                if ((mEnableClipFooterWhenFixedBehind && mRefreshFooter.getSpinnerStyle() == ZeekrSpinnerStyle.FIXED_BEHIND) || mRefreshFooter.getSpinnerStyle().scale) {
                    canvas.save();
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            Log.i(TAG, "computeScroll: " + finalY + " - " + mVerticalPermit);
            if ((finalY < 0 && (mEnableRefresh || mEnableOverScrollDrag) && mZeekrRefreshContent.canRefresh())
                    || (finalY > 0 && (mEnableLoadMore || mEnableOverScrollDrag) && mZeekrRefreshContent.canLoadMore())) {
                if(mVerticalPermit) {
                    float velocity;
    ...
    private static final String TAG = "ZeekrRefreshLayout";
    /**
     * 事件分发 （手势核心）
     * 1.多点触摸
     * 2.无缝衔接内容滚动
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        if (interceptAnimatorByAction(action) || mState.isFinishing
                || (mState == ZeekrRefreshState.Loading && mDisableContentWhenLoading)
                || (mState == ZeekrRefreshState.Refreshing && mDisableContentWhenRefresh)) {
            return false;
        }
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mState == ZeekrRefreshState.TwoLevel && mTouchY < thisView.getMeasuredHeight() * (1 - mTwoLevelBottomPullUpToCloseRate)) {
                    mDragDirection = 'h';//二级刷新标记水平滚动来禁止拖动
                    return mSuperDispatchTouchEvent;
                }
                if (mZeekrRefreshContent != null) {
                    //为 RefreshContent 传递当前触摸事件的坐标，用于智能判断对应坐标位置View的滚动边界和相关信息
                    mZeekrRefreshContent.onActionDown(e);
                }
                return true;
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mIsBeingDragged && !mEnableDisallowIntercept && mDragDirection != 'h' && mZeekrRefreshContent != null) {//没有拖动之前，检测  canRefresh canLoadMore 来开启拖动
                    if (mDragDirection == 'v' || (Math.abs(dy) >= mTouchSlop && Math.abs(dx) < Math.abs(dy))) {//滑动允许最大角度为45度
                        mDragDirection = 'v';
                        if (dy > 0 && (mSpinner < 0 || ((mEnableOverScrollDrag || mEnableRefresh) && mZeekrRefreshContent.canRefresh()))) {
                            mIsBeingDragged = true;
                            mTouchY = touchY - mTouchSlop;//调整 mTouchSlop 偏差
                        } else if (dy < 0 && (mSpinner > 0 || ((mEnableOverScrollDrag || mEnableLoadMore) && ((mState== ZeekrRefreshState.Loading&&mFooterLocked)|| mZeekrRefreshContent.canLoadMore())))) {
                            mIsBeingDragged = true;
                            mTouchY = touchY + mTouchSlop;//调整 mTouchSlop 偏差
                        }
                        if (mIsBeingDragged) {
                            ...
                            mKernel.setState((mSpinner > 0 || (mSpinner == 0 && dy > 0)) ? ZeekrRefreshState.PullDownToRefresh : ZeekrRefreshState.PullUpToLoad);
                ...
                if (mIsBeingDragged) {
                    ...
                    if ((mViceState.isHeader && (spinner < 0 || mLastSpinner < 0)) || (mViceState.isFooter && (spinner > 0 || mLastSpinner > 0))) {
                        ...
                        if (spinner > 0 && ((mEnableOverScrollDrag || mEnableRefresh) && mZeekrRefreshContent.canRefresh())) {
                            mTouchY = mLastTouchY = touchY;
                            mTouchSpinner = spinner = 0;
                            mKernel.setState(ZeekrRefreshState.PullDownToRefresh);
                        } else if (spinner < 0 && ((mEnableOverScrollDrag || mEnableLoadMore) && mZeekrRefreshContent.canLoadMore())) {
                            mTouchY = mLastTouchY = touchY;
                            mTouchSpinner = spinner = 0;
                            mKernel.setState(ZeekrRefreshState.PullUpToLoad);
                        }
    ...
    public void requestDisallowInterceptTouchEvent(boolean disallowIntercept) {
        ...
        // isn't stolen
        View target = mZeekrRefreshContent.getScrollableView();
    ...
    protected boolean startFlingIfNeed(float flingVelocity) {
        ...
        if (Build.VERSION.SDK_INT > 27 && mZeekrRefreshContent != null) {
            /*
        ...
        if (Math.abs(velocity) > mMinimumVelocity) {
            if (velocity * mSpinner < 0) {
                 ...
                 */
                if (mState == ZeekrRefreshState.Refreshing || mState == ZeekrRefreshState.Loading || (mSpinner < 0 && mFooterNoMoreData)) {
            ...
            if ((velocity < 0 && ((mEnableOverScrollBounce && (mEnableLoadMore || mEnableOverScrollDrag)) || (mState == ZeekrRefreshState.Loading && mSpinner >= 0) || (mEnableAutoLoadMore&&isEnableRefreshOrLoadMore(mEnableLoadMore))))
                    || (velocity > 0 && ((mEnableOverScrollBounce && mEnableRefresh || mEnableOverScrollDrag) || (mState == ZeekrRefreshState.Refreshing && mSpinner <= 0)))) {
                /*
    ...
    protected boolean interceptAnimatorByAction(int action) {
        if (action == MotionEvent.ACTION_DOWN) {
            if (reboundAnimator != null) {
                if (mState.isFinishing || mState == ZeekrRefreshState.TwoLevelReleased || mState == ZeekrRefreshState.RefreshReleased || mState == ZeekrRefreshState.LoadReleased) {
                    return true;//完成动画和打开动画不能被打断
                }
                if (mState == ZeekrRefreshState.PullDownCanceled) {
                    mKernel.setState(ZeekrRefreshState.PullDownToRefresh);
                } else if (mState == ZeekrRefreshState.PullUpCanceled) {
                    mKernel.setState(ZeekrRefreshState.PullUpToLoad);
                }
                reboundAnimator.setDuration(0);//cancel会触发End调用，可以判断0来确定是否被cancel
                reboundAnimator.cancel();//会触发 cancel 和 end 调用
     ...
     * @param state 状态
     */
    protected void notifyStateChanged(ZeekrRefreshState state) {
        final ZeekrRefreshState oldState = mState;
        if (oldState != state) {
            mState = state;
            mViceState = state;
            final ZeekrOnStateChangedListener refreshHeader = mRefreshHeader;
            final ZeekrOnStateChangedListener refreshFooter = mRefreshFooter;
            final ZeekrOnStateChangedListener refreshListener = mOnMultiListener;
            ...
            if (state == ZeekrRefreshState.LoadFinish) {
                mFooterLocked = false;
            }
    ...
    protected void setStateDirectLoading(boolean triggerLoadMoreEvent) {
        if (mState != ZeekrRefreshState.Loading) {
...
//            }
            ...
            notifyStateChanged(ZeekrRefreshState.Loading);
            ...
            if (mOnMultiListener != null && mRefreshFooter instanceof ZeekrRefreshFooter) {
                final ZeekrOnLoadMoreListener listener = mOnMultiListener;
                if (triggerLoadMoreEvent) {
                    listener.onLoadMore(this);
                }
                final float maxDragHeight = mFooterMaxDragRate < 10 ? mFooterHeight * mFooterMaxDragRate : mFooterMaxDragRate;
                mOnMultiListener.onFooterStartAnimator((ZeekrRefreshFooter) mRefreshFooter, mFooterHeight, (int) maxDragHeight);
            }
        }
    }
    ...
    protected void setStateLoading(final boolean notify) {
        ...
        notifyStateChanged(ZeekrRefreshState.LoadReleased);
        ...
        if (mOnMultiListener != null && mRefreshFooter instanceof ZeekrRefreshFooter) {
            ...
            final float maxDragHeight = mFooterMaxDragRate < 10 ? mFooterHeight * mFooterMaxDragRate : mFooterMaxDragRate;
            mOnMultiListener.onFooterReleased((ZeekrRefreshFooter) mRefreshFooter, mFooterHeight, (int) maxDragHeight);
        }
        if (animator == null) {
        ...
        // 如果是开启回弹上拉提示，需要手动关闭上拉布局
        if (mEnableOverScrollUpTips) {
            finishLoadMore(0);
        }
    }
    ...
    protected void setStateRefreshing(final boolean notify) {
        ...
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mLastOpenTime = currentTimeMillis();
                notifyStateChanged(ZeekrRefreshState.Refreshing);
                if (mRefreshListener != null) {
                    if(notify) {
                        mRefreshListener.onRefresh(ZeekrRefreshLayout.this);
                    }
                } else if (mOnMultiListener == null) {
                    finishRefresh(3000);
                }
                if (mRefreshHeader != null) {
                    ...
                    mRefreshHeader.onStartAnimator(ZeekrRefreshLayout.this, mHeaderHeight,  maxDragHeight);
                }
                if (mOnMultiListener != null && mRefreshHeader instanceof ZeekrRefreshHeader) {
                    if (notify) {
                        mOnMultiListener.onRefresh(ZeekrRefreshLayout.this);
                    }
                    final int maxDragHeight = (int)(mHeaderMaxDragRate < 10 ? mHeaderHeight * mHeaderMaxDragRate : mHeaderMaxDragRate);
                    mOnMultiListener.onHeaderStartAnimator((ZeekrRefreshHeader) mRefreshHeader, mHeaderHeight,  maxDragHeight);
                }
            }
        };
        notifyStateChanged(ZeekrRefreshState.RefreshReleased);
        // 这里需要做一个判断 如果是回弹加提示模式，需要这个头布局进行辅助
        ValueAnimator animator = mKernel.animSpinner(mEnableOverScrollDownTips ? 0:mHeaderHeight);
        if (animator != null) {
            animator.addListener(listener);
        }
        if (mRefreshHeader != null) {
        ...
        if (mOnMultiListener != null && mRefreshHeader instanceof ZeekrRefreshHeader) {
            ...
            final int maxDragHeight = (int)(mHeaderMaxDragRate < 10 ? mHeaderHeight * mHeaderMaxDragRate : mHeaderMaxDragRate);
            mOnMultiListener.onHeaderReleased((ZeekrRefreshHeader)mRefreshHeader, mHeaderHeight,  maxDragHeight);
        }
        if (animator == null) {
            //onAnimationEnd 会改变状态为 Refreshing 必须在 onReleased 之后调用
            listener.onAnimationEnd(null);
        }
        // 如果是开启了下拉回弹提示的话，需要自动关闭结束header布局
        if (mEnableOverScrollDownTips) {
            finishRefresh(0);
        }
    }
...
//    }
     ...
     * @param state 状态
     */
    protected void setViceState(ZeekrRefreshState state) {
     ...
     * @return enable
     */
    protected boolean isEnableTranslationContent(boolean enable, @Nullable ZeekrRefreshComponent internal) {
        /*
         * 2019-12-25 修复 2.0 版本之后无默认 Header Footer 导致的纯滚动模式无效 添加 @Nullable
         */
        return enable || mEnablePureScrollMode || internal == null || internal.getSpinnerStyle() == ZeekrSpinnerStyle.FIXED_BEHIND;
    }
    ...
    protected class FlingRunnable implements Runnable {
        ...
        public Runnable start() {
            ...
            if (mSpinner != 0 && (!(mState.isOpening || (mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore)))
                    || ((mState == ZeekrRefreshState.Loading || (mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore))) && mSpinner < -mFooterHeight)
                    || (mState == ZeekrRefreshState.Refreshing && mSpinner > mHeaderHeight))) {
                int frame = 0;
                int offset = mSpinner;
                ...
                while (spinner * offset > 0) {
                    ...
                    if (Math.abs(velocityFrame) < 1) {
                        if (!mState.isOpening
                                || (mState == ZeekrRefreshState.Refreshing && offset > mHeaderHeight)
                                || (mState != ZeekrRefreshState.Refreshing && offset < -mFooterHeight)) {
                            return null;
                        }
                        break;
                    }
...
//                mVelocity *= Math.pow(mDamping, ++mFrame);
                ...
                if (Math.abs(velocity) > 1) {
                    ...
                    if (mSpinner * mOffset > 0) {
                        ...
                        animationRunnable = null;
                        mKernel.moveSpinner(0, true);
                        fling(mZeekrRefreshContent.getScrollableView(), (int) -mVelocity);
            ...
            if (animationRunnable == this && !mState.isFinishing) {
                ...
                if (Math.abs(velocity) >= 1) {
                    ...
                    if (mViceState.isDragging && mViceState.isHeader) {
                        mKernel.setState(ZeekrRefreshState.PullDownCanceled);
                    } else if (mViceState.isDragging && mViceState.isFooter) {
                        mKernel.setState(ZeekrRefreshState.PullUpCanceled);
                    }
                    animationRunnable = null;
                    if (Math.abs(mSpinner) >= Math.abs(mSmoothDistance)) {
                        int duration = 10 * Math.min(Math.max((int) ZeekrSmartUtil.px2dp(Math.abs(mSpinner-mSmoothDistance)), 30), 100);
        ...
        if (mSpinner != endSpinner) {
            ...
            reboundAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    reboundAnimator = null;
                    if (mSpinner == 0 && mState != ZeekrRefreshState.None && !mState.isOpening && !mState.isDragging) {
                        notifyStateChanged(ZeekrRefreshState.None);
            ...
            reboundAnimator.addUpdateListener(new AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    mKernel.moveSpinner(animatedValue, false);
                }
            });
...
//            reboundAnimator.setDuration(20000);
    ...
    protected void animSpinnerBounce(final float velocity) {
        Log.i(TAG, "animSpinnerBounce: ");
        if (reboundAnimator == null) {
            if (velocity > 0 && (mState == ZeekrRefreshState.Refreshing || mState == ZeekrRefreshState.TwoLevel)) {
                animationRunnable = new BounceRunnable(velocity, mHeaderHeight);
            } else if (velocity < 0 && (mState == ZeekrRefreshState.Loading
                    || (mEnableFooterFollowWhenNoMoreData && mFooterNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore))
                    || (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && mState != ZeekrRefreshState.Refreshing))) {
                animationRunnable = new BounceRunnable(velocity, -mFooterHeight);
            } else if (mSpinner == 0 && mEnableOverScrollBounce) {
                Log.i(TAG, "animSpinnerBounce: mSpinner == 0 && mEnableOverScrollBounce");
    ...
    protected void overSpinner() {
        Log.i(TAG, "overSpinner: ");
        if (mState == ZeekrRefreshState.TwoLevel) {
            final View thisView = this;
        ...
        } else if (mState == ZeekrRefreshState.Loading
        ...
        } else if (mState == ZeekrRefreshState.Refreshing) {
            if (mSpinner > mHeaderHeight) {
                mKernel.animSpinner(mHeaderHeight);
            } else if (mSpinner < 0) {
                mKernel.animSpinner(0);
            }
        } else if (mState == ZeekrRefreshState.PullDownToRefresh) {
            mKernel.setState(ZeekrRefreshState.PullDownCanceled);
        } else if (mState == ZeekrRefreshState.PullUpToLoad) {
            mKernel.setState(ZeekrRefreshState.PullUpCanceled);
        } else if (mState == ZeekrRefreshState.ReleaseToRefresh) {
            mKernel.setState(ZeekrRefreshState.Refreshing);
        } else if (mState == ZeekrRefreshState.ReleaseToLoad) {
            mKernel.setState(ZeekrRefreshState.Loading);
        } else if (mState == ZeekrRefreshState.ReleaseToTwoLevel) {
            mKernel.setState(ZeekrRefreshState.TwoLevelReleased);
        } else if (mState == ZeekrRefreshState.RefreshReleased) {
            if (reboundAnimator == null) {
                mKernel.animSpinner(mHeaderHeight);
            }
        } else if (mState == ZeekrRefreshState.LoadReleased) {
            if (reboundAnimator == null) {
                mKernel.animSpinner(-mFooterHeight);
            }
        } else if (mState == ZeekrRefreshState.LoadFinish) {
            /*
    ...
    protected void moveSpinnerInfinitely(float spinner) {
        Log.i(TAG, "moveSpinnerInfinitely: " + spinner);
        final View thisView = this;
        if (mNestedInProgress && !mEnableLoadMoreWhenContentNotFull && spinner < 0) {
            if (!mZeekrRefreshContent.canLoadMore()) {
                /*
         ...
         *          源码引用，然后删掉下面4行的代码
         */
        if (mState == ZeekrRefreshState.TwoLevel && spinner > 0) {
            mKernel.moveSpinner(Math.min((int) spinner, thisView.getMeasuredHeight()), true);
        } else if (mState == ZeekrRefreshState.Refreshing && spinner >= 0) {
        ...
        } else if (spinner < 0 && (mState == ZeekrRefreshState.Loading
                || (mEnableFooterFollowWhenNoMoreData && mFooterNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore))
            ...
            final double y = Math.min(M * (1 - Math.pow(100, -x / (H == 0 ? 1 : H))), x);// 公式 y = M(1-100^(-x/H))
            Log.i(TAG, "moveSpinnerInfinitely: " + M + " - " + H + " - " + x + " - " + y);
        ...
        if (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && spinner < 0
                && mState != ZeekrRefreshState.Refreshing
                && mState != ZeekrRefreshState.Loading
                && mState != ZeekrRefreshState.LoadFinish) {
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    if (mLoadMoreListener != null) {
                        mLoadMoreListener.onLoadMore(ZeekrRefreshLayout.this);
                    } else if (mOnMultiListener == null) {
                        finishLoadMore(2000);//如果没有任何加载监听器，两秒之后自动关闭
                    }
                    final ZeekrOnLoadMoreListener listener = mOnMultiListener;
                    if (listener != null) {
                        listener.onLoadMore(ZeekrRefreshLayout.this);
                    }
                }
            }, mReboundDuration);
        }
...
//    }
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ZeekrRefreshLayout_Layout);
            backgroundColor = ta.getColor(R.styleable.ZeekrRefreshLayout_Layout_layout_zeekrRLBackgroundColor, backgroundColor);
            if (ta.hasValue(R.styleable.ZeekrRefreshLayout_Layout_zeekrRLStyle)) {
                spinnerStyle = ZeekrSpinnerStyle.VALUES[ta.getInt(R.styleable.ZeekrRefreshLayout_Layout_zeekrRLStyle, ZeekrSpinnerStyle.TRANSLATE.ordinal)];
            }
            ta.recycle();
        }
...
//        }
        ...
        public ZeekrSpinnerStyle spinnerStyle = null;
    }
    //</editor-fold>
    ...
    public void onNestedScroll(@NonNull View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if ((dy < 0 && (mEnableRefresh || mEnableOverScrollDrag) && (mTotalUnconsumed != 0 || mScrollBoundaryDecider == null || mScrollBoundaryDecider.canRefresh(mZeekrRefreshContent.getView())))
                || (dy > 0 && (mEnableLoadMore || mEnableOverScrollDrag) && (mTotalUnconsumed != 0 || mScrollBoundaryDecider == null || mScrollBoundaryDecider.canLoadMore(mZeekrRefreshContent.getView())))) {
            if (mViceState == ZeekrRefreshState.None || mViceState.isOpening) {
                /*
                 * 嵌套下拉或者上拉时，如果状态还是原始，需要更新到对应的状态
                 * mViceState.isOpening 时，主要修改的也是 mViceState 本身，而 mState 一直都是 isOpening
                 */
                mKernel.setState(dy > 0 ? ZeekrRefreshState.PullUpToLoad : ZeekrRefreshState.PullDownToRefresh);
                if (!scrolled) {
                    final View thisView = this;
                    final ViewParent parent = thisView.getParent();
                    if (parent != null) {
...
//    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setHeaderHeight(float heightDp) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setHeaderHeightPx(int height) {
        if (height == mHeaderHeight) {
            return this;
        }
        if (mHeaderHeightStatus.canReplaceWith(ZeekrDimensionStatus.CodeExact)) {
            mHeaderHeight = height;
            if (mRefreshHeader != null && mAttachedToWindow && mHeaderHeightStatus.notified) {
                ZeekrSpinnerStyle style = mRefreshHeader.getSpinnerStyle();
                if (style != ZeekrSpinnerStyle.MATCH_LAYOUT && !style.scale) {
                    /*
...
//                  mRefreshHeader.getView().requestLayout();
                    ...
                    int top = mlp.topMargin + mHeaderInsetStart - ((style == ZeekrSpinnerStyle.TRANSLATE) ? mHeaderHeight : 0);
                    headerView.layout(left, top, left + headerView.getMeasuredWidth(), top + headerView.getMeasuredHeight());
                }
                final int maxDragHeight = (int)(mHeaderMaxDragRate < 10 ? mHeaderHeight * mHeaderMaxDragRate : mHeaderMaxDragRate);
                mHeaderHeightStatus = ZeekrDimensionStatus.CodeExact;
                mRefreshHeader.onInitialized(mKernel, mHeaderHeight, maxDragHeight);
            } else {
                mHeaderHeightStatus = ZeekrDimensionStatus.CodeExactUnNotify;
            }
        }
        return this;
    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFooterHeight(float heightDp) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFooterHeightPx(int height) {
        if (height == mFooterHeight) {
            return this;
        }
        if (mFooterHeightStatus.canReplaceWith(ZeekrDimensionStatus.CodeExact)) {
            mFooterHeight = height;
            if (mRefreshFooter != null && mAttachedToWindow && mFooterHeightStatus.notified) {
                ZeekrSpinnerStyle style = mRefreshFooter.getSpinnerStyle();
                if (style != ZeekrSpinnerStyle.MATCH_LAYOUT && !style.scale) {
                    /*
                     * 兼容 MotionLayout 2019-6-18
                     * 在 MotionLayout 内部 requestLayout 无效
...
//                  mRefreshFooter.getView().requestLayout();
                    ...
                    final int left = mlp.leftMargin;
                    final int top = mlp.topMargin + thisView.getMeasuredHeight() - mFooterInsetStart - ((style != ZeekrSpinnerStyle.TRANSLATE) ? mFooterHeight : 0);
                    footerView.layout(left, top, left + footerView.getMeasuredWidth(), top + footerView.getMeasuredHeight());
                }
                final float maxDragHeight = mFooterMaxDragRate < 10 ? mFooterHeight * mFooterMaxDragRate : mFooterMaxDragRate;
                mFooterHeightStatus = ZeekrDimensionStatus.CodeExact;
                mRefreshFooter.onInitialized(mKernel, mFooterHeight, (int) maxDragHeight);
            } else {
                mFooterHeightStatus = ZeekrDimensionStatus.CodeExactUnNotify;
            }
        }
        return this;
    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setHeaderInsetStart(float insetDp) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setHeaderInsetStartPx(int insetPx) {
     ...
     * @see com.zeekr.component.refresh.api.ZeekrRefreshLayout#setHeaderInsetStart(float)
     * @param insetDp Density-independent Pixels 虚拟像素（px需要调用px2dp转换）
     * @return RefreshLayout
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFooterInsetStart(float insetDp) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFooterInsetStartPx(int insetPx) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setDragRate(float rate) {
        this.mDragRate = rate;
        return this;
    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setHeaderMaxDragRate(float rate) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFooterMaxDragRate(float rate) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setHeaderTriggerRate(float rate) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFooterTriggerRate(float rate) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setReboundInterpolator(@NonNull Interpolator interpolator) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setReboundDuration(int duration) {
    ...
    /**
     * 设置是否开启回弹下拉时有文本提示
     * @param enableOverScrollDownTips true 开启回弹下拉文本提示  false 关闭回弹下拉文本提示
     * @return ZeekrRefreshLayout
     */
    public ZeekrRefreshLayout setEnableOverScrollDownTips(boolean enableOverScrollDownTips) {
        this.mEnableOverScrollDownTips = enableOverScrollDownTips;
        return this;
    }

    /**
     * 设置是否开启回弹上拉时文本提示
     * @param enableOverScrollUpTips true 开启回弹上拉文本提示 false 关闭回弹上拉文本提示
     * @return ZeekrRefreshLayout
     */
    public ZeekrRefreshLayout setEnableOverScrollUpTips(boolean enableOverScrollUpTips) {
        this.mEnableOverScrollUpTips = enableOverScrollUpTips;
        return this;
    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableLoadMore(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableRefresh(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableHeaderTranslationContent(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableFooterTranslationContent(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableAutoLoadMore(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableOverScrollBounce(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnablePureScrollMode(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableScrollContentWhenLoaded(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableScrollContentWhenRefreshed(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableLoadMoreWhenContentNotFull(boolean enabled) {
        this.mEnableLoadMoreWhenContentNotFull = enabled;
        if (mZeekrRefreshContent != null) {
            mZeekrRefreshContent.setEnableLoadMoreWhenContentNotFull(enabled);
        }
        return this;
    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableOverScrollDrag(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableFooterFollowWhenNoMoreData(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableClipHeaderWhenFixedBehind(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableClipFooterWhenFixedBehind(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setEnableNestedScroll(boolean enabled) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFixedHeaderViewId(int id) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFixedFooterViewId(int id) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setHeaderTranslationViewId(int id) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setFooterTranslationViewId(int id) {
...
//    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setDisableContentWhenRefresh(boolean disable) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setDisableContentWhenLoading(boolean disable) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setRefreshHeader(@NonNull ZeekrRefreshHeader header) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setRefreshHeader(@NonNull ZeekrRefreshHeader header, int width, int height) {
        if (mRefreshHeader != null) {
        ...
        this.mHeaderHeightStatus = ZeekrDimensionStatus.DefaultUnNotify;//2020-5-23 修复动态切换时，不能及时测量新的高度
        /*
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setRefreshFooter(@NonNull ZeekrRefreshFooter footer) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setRefreshFooter(@NonNull ZeekrRefreshFooter footer, int width, int height) {
        if (mRefreshFooter != null) {
        ...
        this.mFooterHeightStatus = ZeekrDimensionStatus.DefaultUnNotify;//2020-5-23 修复动态切换时，不能及时测量新的高度
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setRefreshContent(@NonNull View content) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setRefreshContent(@NonNull View content, int width, int height) {
        final View thisView = this;
        if (mZeekrRefreshContent != null) {
            super.removeView(mZeekrRefreshContent.getView());
        }
        final ViewGroup thisGroup = this;
        ...
        mZeekrRefreshContent = new ZeekrRefreshContentWrapper(content);
    ...
    public ZeekrRefreshFooter getRefreshFooter() {
        return mRefreshFooter instanceof ZeekrRefreshFooter ? (ZeekrRefreshFooter) mRefreshFooter : null;
    }
    ...
    public ZeekrRefreshHeader getRefreshHeader() {
        return mRefreshHeader instanceof ZeekrRefreshHeader ? (ZeekrRefreshHeader) mRefreshHeader : null;
    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setOnRefreshListener(ZeekrOnRefreshListener listener) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setOnLoadMoreListener(ZeekrOnLoadMoreListener listener) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setOnRefreshLoadMoreListener(ZeekrOnRefreshLoadMoreListener listener) {
        this.mRefreshListener = listener;
        this.mLoadMoreListener = listener;
        this.mEnableLoadMore = mEnableLoadMore || (!mManualLoadMore && listener != null);
        return this;
     ...
     * Recommended {@link ZeekrSimpleBoundaryDecider}
     * 设置滚动边界判断器
     * 建议使用 {@link ZeekrSimpleBoundaryDecider}
     * @param listener OnMultiListener 多功能监听器
     * @return RefreshLayout
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setOnMultiListener(ZeekrOnMultiListener listener) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setPrimaryColors(@ColorInt int... primaryColors) {
        if (mRefreshHeader != null) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setPrimaryColorsId(@ColorRes int... primaryColorId) {
        final View thisView = this;
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setScrollBoundaryDecider(ZeekrScrollBoundaryDecider boundary) {
        mScrollBoundaryDecider = boundary;
        if (mZeekrRefreshContent != null) {
            mZeekrRefreshContent.setScrollBoundaryDecider(boundary);
        }
        return this;
    }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout setNoMoreData(boolean noMoreData) {
        if (mState == ZeekrRefreshState.Refreshing && noMoreData) {
            finishRefreshWithNoMoreData();
        } else if (mState == ZeekrRefreshState.Loading && noMoreData) {
            finishLoadMoreWithNoMoreData();
        } else if (mFooterNoMoreData != noMoreData) {
            mFooterNoMoreData = noMoreData;
            if (mRefreshFooter instanceof ZeekrRefreshFooter) {
                if (((ZeekrRefreshFooter) mRefreshFooter).setNoMoreData(noMoreData)) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout resetNoMoreData() {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishRefresh() {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishLoadMore() {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishRefresh(int delayed) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishRefresh(boolean success) {
        if (success) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishRefresh(final int delayed, final boolean success, final Boolean noMoreData) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                if (count == 0) {
                    if (mState == ZeekrRefreshState.None && mViceState == ZeekrRefreshState.Refreshing) {
                        //autoRefresh 即将执行，但未开始
                        mViceState = ZeekrRefreshState.None;
                    } else if (reboundAnimator != null && mState.isHeader && (mState.isDragging || mState == ZeekrRefreshState.RefreshReleased)) {
                        ...
                        if (mKernel.animSpinner(0) == null) {
                            ...
                            notifyStateChanged(ZeekrRefreshState.PullDownCanceled);
                        }
//                      mKernel.setState(RefreshState.None);
                    } else if (mState == ZeekrRefreshState.Refreshing && mRefreshHeader != null && mZeekrRefreshContent != null) {
                        count++;
                        mHandler.postDelayed(this, more);
                        //提前设置 状态为 RefreshFinish 防止 postDelayed 导致 finishRefresh 过后，外部判断 state 还是 Refreshing
                        notifyStateChanged(ZeekrRefreshState.RefreshFinish);
                    ...
                    int startDelay = mRefreshHeader.onFinish(ZeekrRefreshLayout.this, success);
                    if (mOnMultiListener != null && mRefreshHeader instanceof ZeekrRefreshHeader) {
                        mOnMultiListener.onHeaderFinish((ZeekrRefreshHeader) mRefreshHeader, success);
                    }
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                mTouchSpinner = 0;
                                mIsBeingDragged = false;
                                ZeekrRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                                ZeekrRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                ZeekrRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                                mNestedInProgress = false;
                                mTouchSpinner = 0;
                            }
                        }
                        if (mSpinner > 0) {
                            ...
                            if (mEnableScrollContentWhenRefreshed) {
                                updateListener = mZeekrRefreshContent.scrollContentWhenFinished(mSpinner);
                            }
...
//                            resetStatus();
                            mKernel.setState(ZeekrRefreshState.None);
                        }
                    }
                }
            }
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishRefreshWithNoMoreData() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishLoadMore(int delayed) {
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishLoadMore(boolean success) {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishLoadMore(final int delayed, final boolean success, final boolean noMoreData) {
        final int more = delayed >> 16;//动画剩余延时
        int delay = delayed << 16 >> 16;//用户指定延时
        Runnable runnable = new Runnable() {
            int count = 0;
            ...
            public void run() {
                if (count == 0) {
                    if (mState == ZeekrRefreshState.None && mViceState == ZeekrRefreshState.Loading) {
                        //autoLoadMore 即将执行，但未开始
                        mViceState = ZeekrRefreshState.None;
                    } else if (reboundAnimator != null && (mState.isDragging || mState == ZeekrRefreshState.LoadReleased) && mState.isFooter) {
                        ...
                        if (mKernel.animSpinner(0) == null) {
                            ...
                            notifyStateChanged(ZeekrRefreshState.PullUpCanceled);
                        }
                        //mKernel.setState(RefreshState.None);
                    } else if (mState == ZeekrRefreshState.Loading && mRefreshFooter != null && mZeekrRefreshContent != null) {
                        count++;
                        mHandler.postDelayed(this, more);
                        //提前设置 状态为 LoadFinish 防止 postDelayed 导致 finishLoadMore 过后，外部判断 state 还是 Loading
                        notifyStateChanged(ZeekrRefreshState.LoadFinish);
                        return;
                    }
                    if (noMoreData) {
                        setNoMoreData(true);
                    ...
                    final int startDelay = mRefreshFooter.onFinish(ZeekrRefreshLayout.this, success);
                    if (mOnMultiListener != null && mRefreshFooter instanceof ZeekrRefreshFooter) {
                        mOnMultiListener.onFooterFinish((ZeekrRefreshFooter) mRefreshFooter, success);
                    }
                    if (startDelay < Integer.MAX_VALUE) {
                        //计算布局将要移动的偏移量
                        final boolean needHoldFooter = noMoreData && mEnableFooterFollowWhenNoMoreData && mSpinner < 0 && mZeekrRefreshContent.canLoadMore();
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                ZeekrRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + offsetY + mTouchSlop * 2, 0));
                                ZeekrRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offsetY, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                ZeekrRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                            ...
                            public void run() {
                                ...
                                AnimatorListenerAdapter listenerAdapter = new AnimatorListenerAdapter() {
                                    ...
                                    public void onAnimationEnd(Animator animation) {
                                        ...
                                        if (mState == ZeekrRefreshState.LoadFinish) {
                                            notifyStateChanged(ZeekrRefreshState.None);
                                        }
                                    }
                                };
                                if (mSpinner > 0) { //大于0表示下拉, 这是 Header 可见, Footer 不可见
                                    ...
                                    if (noMoreData && mEnableFooterFollowWhenNoMoreData) {//如果需要显示没有更多数据
                                        if (mSpinner >= -mFooterHeight) {//如果 Footer 的位置再可见范围内
                                            notifyStateChanged(ZeekrRefreshState.None);//直接通知重置状态,不关闭 Footer
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout finishLoadMoreWithNoMoreData() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public com.zeekr.component.refresh.api.ZeekrRefreshLayout closeHeaderOrFooter() {
        if (mState == ZeekrRefreshState.None && (mViceState == ZeekrRefreshState.Refreshing || mViceState == ZeekrRefreshState.Loading)) {
            //autoRefresh autoLoadMore 即将执行，但未开始
            mViceState = ZeekrRefreshState.None;
        }
        if (mState == ZeekrRefreshState.Refreshing) {
            finishRefresh();
        } else if (mState == ZeekrRefreshState.Loading) {
            finishLoadMore();
        } else {
    ...
    public boolean autoRefresh(int delayed, final int duration, final float dragRate,final boolean animationOnly) {
        if (mState == ZeekrRefreshState.None && isEnableRefreshOrLoadMore(mEnableRefresh)) {
            Runnable runnable = new Runnable() {
                @Override
                public void run() {
                    if (mViceState != ZeekrRefreshState.Refreshing) return;
                    if (reboundAnimator != null) {
                        reboundAnimator.setDuration(0);//cancel会触发End调用，可以判断0来确定是否被cancel
                        reboundAnimator.cancel();//会触发 cancel 和 end 调用
                        reboundAnimator = null;
                    ...
                    final View thisView = ZeekrRefreshLayout.this;
                    mLastTouchX = thisView.getMeasuredWidth() / 2f;
                    mKernel.setState(ZeekrRefreshState.PullDownToRefresh);
                    ...
                    reboundAnimator.setDuration(duration);
                    reboundAnimator.setInterpolator(new ZeekrSmartUtil(ZeekrSmartUtil.INTERPOLATOR_VISCOUS_FLUID));
                    reboundAnimator.addUpdateListener(new AnimatorUpdateListener() {
                        @Override
                        public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    reboundAnimator.addListener(new AnimatorListenerAdapter() {
                        ...
                        public void onAnimationEnd(Animator animation) {
                            ...
                            reboundAnimator = null;
                            if (mRefreshHeader != null) {
                                if (mState != ZeekrRefreshState.ReleaseToRefresh) {
                                    mKernel.setState(ZeekrRefreshState.ReleaseToRefresh);
                                }
            ...
            setViceState(ZeekrRefreshState.Refreshing);
            if (delayed > 0) {
    ...
    public boolean autoLoadMore(int delayed, final int duration, final float dragRate, final boolean animationOnly) {
        if (mState == ZeekrRefreshState.None && (isEnableRefreshOrLoadMore(mEnableLoadMore) && !mFooterNoMoreData)) {
            Runnable runnable = new Runnable() {
                @Override
                public void run() {
                    if (mViceState != ZeekrRefreshState.Loading)return;
                    if (reboundAnimator != null) {
                        reboundAnimator.setDuration(0);//cancel会触发End调用，可以判断0来确定是否被cancel
                        reboundAnimator.cancel();//会触发 cancel 和 end 调用
                        reboundAnimator = null;
                    ...
                    reboundAnimator.addListener(new AnimatorListenerAdapter() {
                        ...
                        public void onAnimationEnd(Animator animation) {
                            ...
                            reboundAnimator = null;
                            if (mRefreshFooter != null) {
                                if (mState != ZeekrRefreshState.ReleaseToLoad) {
                                    mKernel.setState(ZeekrRefreshState.ReleaseToLoad);
                                }
            ...
            setViceState(ZeekrRefreshState.Loading);
            if (delayed > 0) {
                mHandler.postDelayed(runnable, delayed);
            } else {
                runnable.run();
     ...
     */
    public static void setDefaultRefreshHeaderCreator(@NonNull ZeekrDefaultRefreshHeaderCreator creator) {
     ...
     */
    public static void setDefaultRefreshFooterCreator(@NonNull ZeekrDefaultRefreshFooterCreator creator) {
     ...
     */
    public static void setDefaultRefreshInitializer(@NonNull ZeekrDefaultRefreshInitializer initializer) {
    ...
    public boolean isRefreshing() {
        return mState == ZeekrRefreshState.Refreshing;
    }
    ...
    public boolean isLoading() {
        return mState == ZeekrRefreshState.Loading;
    }
    //</editor-fold>
    //</editor-fold>
    ...
    public class ZeekrRefreshKernelImpl implements ZeekrRefreshKernel {
        ...
        @NonNull
        @Override
        public com.zeekr.component.refresh.api.ZeekrRefreshLayout getRefreshLayout() {
            return ZeekrRefreshLayout.this;
        }

        @NonNull
        @Override
        public ZeekrRefreshContent getRefreshContent() {
            return mZeekrRefreshContent;
        }

        //<editor-fold desc="状态更改 state changes">
        @Override
        public ZeekrRefreshKernel setState(@NonNull ZeekrRefreshState state) {
            switch (state) {
                case None:
                    if (mState != ZeekrRefreshState.None && mSpinner == 0) {
                        notifyStateChanged(ZeekrRefreshState.None);
                    } else if (mSpinner != 0) {
                        animSpinner(0);
                    }
                ...
                case PullDownToRefresh:
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(ZeekrRefreshState.PullDownToRefresh);
                    } else {
                        setViceState(ZeekrRefreshState.PullDownToRefresh);
                    }
                    break;
                case PullUpToLoad:
                    if (isEnableRefreshOrLoadMore(mEnableLoadMore) && !mState.isOpening && !mState.isFinishing && !(mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective)) {
                        notifyStateChanged(ZeekrRefreshState.PullUpToLoad);
                    } else {
                        setViceState(ZeekrRefreshState.PullUpToLoad);
                    }
                    break;
                case PullDownCanceled:
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(ZeekrRefreshState.PullDownCanceled);
//                        resetStatus();
                        setState(ZeekrRefreshState.None);
                    } else {
                        setViceState(ZeekrRefreshState.PullDownCanceled);
                    }
                    break;
                case PullUpCanceled:
...
//                        resetStatus();
                        ...
                        setViceState(ZeekrRefreshState.PullUpCanceled);
                    }
                    break;
                case ReleaseToRefresh:
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(ZeekrRefreshState.ReleaseToRefresh);
                    } else {
                        setViceState(ZeekrRefreshState.ReleaseToRefresh);
                    }
                    break;
                case ReleaseToLoad:
                    if (isEnableRefreshOrLoadMore(mEnableLoadMore) && !mState.isOpening && !mState.isFinishing && !(mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective)) {
                        notifyStateChanged(ZeekrRefreshState.ReleaseToLoad);
                    } else {
                        setViceState(ZeekrRefreshState.ReleaseToLoad);
                    }
                    break;
                case ReleaseToTwoLevel: {
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(ZeekrRefreshState.ReleaseToTwoLevel);
                    } else {
                        setViceState(ZeekrRefreshState.ReleaseToTwoLevel);
                    }
                    break;
                }
                case RefreshReleased: {
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        ...
                        setViceState(ZeekrRefreshState.RefreshReleased);
                    }
                    break;
                }
                case LoadReleased: {
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableLoadMore)) {
                        ...
                        setViceState(ZeekrRefreshState.LoadReleased);
                    }
                    break;
                }
                case Refreshing:
                    setStateRefreshing(true && !mEnableOverScrollDownTips);
                    break;
                case Loading:
                    setStateLoading(true && !mEnableOverScrollUpTips);
                    break;
                default:
...
//                    break;
        ...
        @Override
        public ZeekrRefreshKernel startTwoLevel(boolean open) {
            if (open) {
                AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        ...
                        mKernel.setState(ZeekrRefreshState.TwoLevel);
                    }
                };
                final View thisView = ZeekrRefreshLayout.this;
        ...
        @Override
        public ZeekrRefreshKernel finishTwoLevel() {
            if (mState == ZeekrRefreshState.TwoLevel) {
                mKernel.setState(ZeekrRefreshState.TwoLevelFinish);
                if (mSpinner == 0) {
                    moveSpinner(0, false);
                    notifyStateChanged(ZeekrRefreshState.None);
                } else {
         ...
         */
        public ZeekrRefreshKernel moveSpinner(final int spinner, final boolean isDragging) {
            if (mSpinner == spinner
            ...
            if (isDragging && (mViceState.isDragging || mViceState.isOpening)) {
                if (mSpinner > (mHeaderTriggerRate < 10 ? mHeaderHeight * mHeaderTriggerRate : mHeaderTriggerRate)) {
                    if (mState != ZeekrRefreshState.ReleaseToTwoLevel) {
                        mKernel.setState(ZeekrRefreshState.ReleaseToRefresh);
                    }
            ...
            if (mZeekrRefreshContent != null) {
                ...
                if (changed) {
                    mZeekrRefreshContent.moveSpinner(tSpinner, mHeaderTranslationViewId, mFooterTranslationViewId);
                    if (mFooterNoMoreData && mFooterNoMoreDataEffective && mEnableFooterFollowWhenNoMoreData
                            && mRefreshFooter instanceof ZeekrRefreshFooter && mRefreshFooter.getSpinnerStyle() == ZeekrSpinnerStyle.TRANSLATE
                            && isEnableRefreshOrLoadMore(mEnableLoadMore)) {
                        mRefreshFooter.getView().setTranslationY(Math.max(0, tSpinner));
                    }
                    boolean header = mEnableClipHeaderWhenFixedBehind && mRefreshHeader != null && mRefreshHeader.getSpinnerStyle() == ZeekrSpinnerStyle.FIXED_BEHIND;
                    header = header || mHeaderBackgroundColor != 0;
                    boolean footer = mEnableClipFooterWhenFixedBehind && mRefreshFooter != null && mRefreshFooter.getSpinnerStyle() == ZeekrSpinnerStyle.FIXED_BEHIND;
            ...
            if ((spinner >= 0 || oldSpinner > 0) && mRefreshHeader != null) {
                ...
                if (isEnableRefreshOrLoadMore(mEnableRefresh) || (mState == ZeekrRefreshState.RefreshFinish && !isDragging)) {
...
//                            mRefreshHeader.getView().requestLayout();
                ...
                if (oldSpinner != mSpinner && mOnMultiListener != null && mRefreshHeader instanceof ZeekrRefreshHeader) {
                    mOnMultiListener.onHeaderMoving((ZeekrRefreshHeader) mRefreshHeader, isDragging, percent, offset, headerHeight, maxDragHeight);
                }
            }
            if ((spinner <= 0 || oldSpinner < 0) && mRefreshFooter != null) {
                ...
                if (isEnableRefreshOrLoadMore(mEnableLoadMore) || (mState == ZeekrRefreshState.LoadFinish && !isDragging)) {
                    if (oldSpinner != mSpinner) {
                        if (mRefreshFooter.getSpinnerStyle() == ZeekrSpinnerStyle.TRANSLATE) {
                            mRefreshFooter.getView().setTranslationY(mSpinner);
...
//                            mRefreshFooter.getView().requestLayout();
                ...
                if (oldSpinner != mSpinner && mOnMultiListener != null && mRefreshFooter instanceof ZeekrRefreshFooter) {
                    mOnMultiListener.onFooterMoving((ZeekrRefreshFooter)mRefreshFooter, isDragging, percent, offset, footerHeight, maxDragHeight);
                }
            }
            return this;
        }
        ...
        public ValueAnimator animSpinner(int endSpinner) {
            return ZeekrRefreshLayout.this.animSpinner(endSpinner, 0, mReboundInterpolator, mReboundDuration);
        }
        //</editor-fold>
        ...
        @Override
        public ZeekrRefreshKernel requestDrawBackgroundFor(@NonNull ZeekrRefreshComponent internal, int backgroundColor) {
        ...
        @Override
        public ZeekrRefreshKernel requestNeedTouchEventFor(@NonNull ZeekrRefreshComponent internal, boolean request) {
            if (internal.equals(mRefreshHeader)) {
        ...
        @Override
        public ZeekrRefreshKernel requestDefaultTranslationContentFor(@NonNull ZeekrRefreshComponent internal, boolean translation) {
            if (internal.equals(mRefreshHeader)) {
        ...
        @Override
        public ZeekrRefreshKernel requestRemeasureHeightFor(@NonNull ZeekrRefreshComponent internal) {
            if (internal.equals(mRefreshHeader)) {
                if (mHeaderHeightStatus.notified) {
                    mHeaderHeightStatus = mHeaderHeightStatus.unNotify();
                }
        ...
        @Override
        public ZeekrRefreshKernel requestFloorDuration(int duration) {
            mFloorDuration = duration;
            return this;
        }

        @Override
        public ZeekrRefreshKernel requestFloorBottomPullUpToCloseRate(float rate) {
