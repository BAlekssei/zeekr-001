 ...
 * SmartUtil
 */
public class ZeekrSmartUtil implements Interpolator {
    ...
    public ZeekrSmartUtil(int type) {
        this.type = type;
    }
    ...
    public static boolean isScrollableView(View view) {
        if (view instanceof ZeekrRefreshComponent) {
            return false;
        }
        ...
        if (targetView instanceof ViewGroup && touch != null) {
            ...
            for (int i = childCount; i > 0; i--) {
                ...
                if (isTransformedTouchPointInView(viewGroup, child, touch.x, touch.y, point)) {
                    Object tag = child.getTag(R.id.zrl_tag);
    ...
    public static boolean canLoadMore(@NonNull View targetView, PointF touch, boolean contentFull) {
        ...
        if (targetView instanceof ViewGroup && touch != null && !ZeekrSmartUtil.isScrollableView(targetView)) {
            ViewGroup viewGroup = (ViewGroup) targetView;
            final int childCount = viewGroup.getChildCount();
            PointF point = new PointF();
            for (int i = childCount; i > 0; i--) {
...
//        final boolean isInView = pointInView(child, point[0], point[1], 0);
    ...
    // 屏幕宽度
    private static float widthPixels = Resources.getSystem().getDisplayMetrics().widthPixels;

    // 屏幕高度
    private static float heightPixels = Resources.getSystem().getDisplayMetrics().heightPixels;
    ...
    public float getInterpolation(float input) {
        ...
        // Log.i("SmartRefreshLayout", "getInterpolation: " + input + " ==== " + interpolated  + " ==== " + VISCOUS_FLUID_NORMALIZE + " ===== " + viscousFluid(input) + " ===== " + interpolated + VISCOUS_FLUID_OFFSET);
        if (interpolated > 0) {
